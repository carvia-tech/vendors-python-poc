...
pipeline {
    ...
    environment {
        ...
        CUSTOM_BUILD_NUMBER = "${getEnvironmentValue().cdk8sEnv}-${BUILD_NUMBER}-${env.GIT_COMMIT.substring(0, 7)}"
        REPOSITORY = 'vendors-python-poc'
        CI = 'true'
        DEPLOY_ENV = getEnvironmentValue()
    }
    ...
    stages {
        ...
        stage('Build Docker Image') {
            steps {
                sh "docker build . -t docker.carvia.tech/${Environment.DEV.dockerRepo}/${REPOSITORY}:${CUSTOM_BUILD_NUMBER}"
            }
        }
...
//                branch 'develop'
            ...
            steps {
                withCredentials([file(credentialsId: "${Environment.DEV.k8sCredentialsId}", variable: 'KUBECONFIG')]) {
                    ...
                    sh "microk8s kubectl --kubeconfig=${KUBECONFIG} apply -f cdk8s/dist/ -n ${Environment.DEV.namespace}"
                }
            }
        }
...
//                }
            ...
            steps {
                withCredentials([file(credentialsId: "${Environment.PRODUCTION.k8sCredentialsId}", variable: 'KUBECONFIG')]) {
                    ...
                    sh "microk8s kubectl --kubeconfig=${KUBECONFIG} apply -f cdk8s/dist/ -n ${Environment.PRODUCTION.namespace}"
                }
            }
        }
    }
    ...
    post {
        always {
            ...
            alertSensePublisher(
                    sourceEventId: 'vendors.python-poc-build',
                    project: 'Vendors Python Service',
